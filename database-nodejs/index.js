...
async function connect(){
    try{
        ...
        const resultDelete = await conn.query(
            "delete from employees where name = ?",["Ashwini Mishra"]
        )
        ...
        const resultUpdate = await conn.query(
            "update employees set name = ? where name = ?",["Ashwini Mishra","Ashwini"]
        );
        ...
        const ssn = "116"
        const emp = await conn.query(`select * from employees where ssn = ?`,[ssn]);
        console.table(emp[0]);
    }
    catch(ex){
        console.error(ex);
